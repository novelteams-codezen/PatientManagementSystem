...
{
    /// <summary>
    /// Controller responsible for managing tokenmanagement-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class TokenManagementController : ControllerBase
    {
        ...
        public TokenManagementController(PatientManagementSystemContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new tokenmanagement to the database</summary>
        ...
        {
            _context.TokenManagement.Add(model);
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.TokenManagement.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<TokenManagement>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.TokenManagement.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.TokenManagement.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] TokenManagement updatedEntity)
        {
            ...
            this._context.TokenManagement.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<TokenManagement> updatedEntity)
        {
            ...
            var existingEntity = this._context.TokenManagement.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.TokenManagement.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
