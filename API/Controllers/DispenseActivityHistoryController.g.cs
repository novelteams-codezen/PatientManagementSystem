...
{
    /// <summary>
    /// Controller responsible for managing dispenseactivityhistory-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class DispenseActivityHistoryController : ControllerBase
    {
        ...
        public DispenseActivityHistoryController(PatientManagementSystemContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new dispenseactivityhistory to the database</summary>
        ...
        {
            _context.DispenseActivityHistory.Add(model);
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.DispenseActivityHistory.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<DispenseActivityHistory>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.DispenseActivityHistory.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.DispenseActivityHistory.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] DispenseActivityHistory updatedEntity)
        {
            ...
            this._context.DispenseActivityHistory.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<DispenseActivityHistory> updatedEntity)
        {
            ...
            var existingEntity = this._context.DispenseActivityHistory.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.DispenseActivityHistory.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
