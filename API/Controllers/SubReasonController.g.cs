...
{
    /// <summary>
    /// Controller responsible for managing subreason-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    [Route("api/subreason")]
    [Authorize]
    public class SubReasonController : ControllerBase
    {
        private readonly PatientManagementSystemContext _context;

        public SubReasonController(PatientManagementSystemContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new subreason to the database</summary>
        ...
        [UserAuthorize("SubReason",Entitlements.Create)]
        public IActionResult Post([FromBody] SubReason model)
        {
            _context.SubReason.Add(model);
        ...
        [HttpGet]
        [UserAuthorize("SubReason",Entitlements.Read)]
        [ProducesResponseType(StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status401Unauthorized)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.SubReason.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<SubReason>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.SubReason.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        /// <returns>The result of the operation</returns>
        [HttpDelete]
        [UserAuthorize("SubReason",Entitlements.Delete)]
        [ProducesResponseType(StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status401Unauthorized)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.SubReason.Remove(entityData);
        ...
        /// <returns>The result of the operation</returns>
        [HttpPut]
        [UserAuthorize("SubReason",Entitlements.Update)]
        [Route("{id:Guid}")]
        [ProducesResponseType(StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status401Unauthorized)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        ...
        public IActionResult UpdateById(Guid id, [FromBody] SubReason updatedEntity)
        {
            ...
            this._context.SubReason.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<SubReason> updatedEntity)
        {
            ...
            var existingEntity = this._context.SubReason.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.SubReason.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
