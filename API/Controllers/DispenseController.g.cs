...
{
    /// <summary>
    /// Controller responsible for managing dispense-related operations in the API.
    /// </summary>
    /// <remarks>
    /// This controller provides endpoints for adding, retrieving, updating, and deleting dispense information.
    /// </remarks>
    [Route("api/dispense")]
    [Authorize]
    public class DispenseController : ControllerBase
    {
        ...
        public DispenseController(PatientManagementSystemContext context)
        {
            _context = context;
        }
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var result = FilterService<Dispense>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.Dispense.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        /// <returns>The result of the operation</returns>
        [HttpDelete]
        [UserAuthorize("Dispense",Entitlements.Delete)]
        [ProducesResponseType(StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status401Unauthorized)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.Dispense.Remove(entityData);
        ...
        /// <returns>The result of the operation</returns>
        [HttpPut]
        [UserAuthorize("Dispense",Entitlements.Update)]
        [Route("{id:Guid}")]
        [ProducesResponseType(StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status401Unauthorized)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        [Produces("application/json")]
        public IActionResult UpdateById(Guid id, [FromBody] Dispense updatedEntity)
        {
            ...
            this._context.Dispense.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        [Produces("application/json")]
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<Dispense> updatedEntity)
        {
            if (updatedEntity == null)
                return BadRequest("Patch document is missing.");
            var existingEntity = this._context.Dispense.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.Dispense.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
