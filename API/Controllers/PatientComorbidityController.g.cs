...
namespace PatientManagementSystem.Controllers
{
    ...
    public class PatientComorbidityController : ControllerBase
    {
        ...
        public PatientComorbidityController(PatientManagementSystemContext context)
        {
            _context = context;
        }
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var result = FilterService<PatientComorbidity>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.PatientComorbidity.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<PatientComorbidity> updatedEntity)
        {
            ...
            var existingEntity = this._context.PatientComorbidity.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.PatientComorbidity.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
