...
namespace PatientManagementSystem.Controllers
{
    ...
    public class ClinicalParameterController : ControllerBase
    {
        ...
        public ClinicalParameterController(PatientManagementSystemContext context)
        {
            _context = context;
        }
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var result = FilterService<ClinicalParameter>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<ClinicalParameter> updatedEntity)
        {
            ...
            var existingEntity = this._context.ClinicalParameter.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.ClinicalParameter.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
