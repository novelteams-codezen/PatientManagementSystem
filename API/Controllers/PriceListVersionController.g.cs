...
{
    /// <summary>
    /// Controller responsible for managing pricelistversion-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class PriceListVersionController : ControllerBase
    {
        ...
        public PriceListVersionController(PatientManagementSystemContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new pricelistversion to the database</summary>
        ...
        {
            _context.PriceListVersion.Add(model);
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.PriceListVersion.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<PriceListVersion>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.PriceListVersion.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.PriceListVersion.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] PriceListVersion updatedEntity)
        {
            ...
            this._context.PriceListVersion.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<PriceListVersion> updatedEntity)
        {
            ...
            var existingEntity = this._context.PriceListVersion.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.PriceListVersion.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
