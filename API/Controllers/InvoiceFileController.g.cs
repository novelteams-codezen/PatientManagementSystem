...
{
    /// <summary>
    /// Controller responsible for managing invoicefile-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class InvoiceFileController : ControllerBase
    {
        ...
        public InvoiceFileController(PatientManagementSystemContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new invoicefile to the database</summary>
        ...
        {
            _context.InvoiceFile.Add(model);
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.InvoiceFile.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<InvoiceFile>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.InvoiceFile.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.InvoiceFile.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] InvoiceFile updatedEntity)
        {
            ...
            this._context.InvoiceFile.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<InvoiceFile> updatedEntity)
        {
            ...
            var existingEntity = this._context.InvoiceFile.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.InvoiceFile.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
