...
{
    /// <summary>
    /// Controller responsible for managing stockadjustmentfile-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class StockAdjustmentFileController : ControllerBase
    {
        ...
        public StockAdjustmentFileController(PatientManagementSystemContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new stockadjustmentfile to the database</summary>
        ...
        {
            _context.StockAdjustmentFile.Add(model);
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.StockAdjustmentFile.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<StockAdjustmentFile>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.StockAdjustmentFile.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.StockAdjustmentFile.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] StockAdjustmentFile updatedEntity)
        {
            ...
            this._context.StockAdjustmentFile.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<StockAdjustmentFile> updatedEntity)
        {
            ...
            var existingEntity = this._context.StockAdjustmentFile.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.StockAdjustmentFile.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
