...
{
    /// <summary>
    /// Controller responsible for managing pricelist-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class PriceListController : ControllerBase
    {
        ...
        public PriceListController(PatientManagementSystemContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new pricelist to the database</summary>
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.PriceList.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<PriceList>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.PriceList.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.PriceList.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] PriceList updatedEntity)
        {
            ...
            this._context.PriceList.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<PriceList> updatedEntity)
        {
            ...
            var existingEntity = this._context.PriceList.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.PriceList.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
