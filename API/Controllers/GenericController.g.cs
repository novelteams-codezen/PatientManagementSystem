...
{
    /// <summary>
    /// Controller responsible for managing generic-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class GenericController : ControllerBase
    {
        ...
        public GenericController(PatientManagementSystemContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new generic to the database</summary>
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.Generic.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<Generic>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.Generic.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.Generic.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] Generic updatedEntity)
        {
            ...
            this._context.Generic.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<Generic> updatedEntity)
        {
            ...
            var existingEntity = this._context.Generic.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.Generic.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
