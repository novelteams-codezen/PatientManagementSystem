...
{
    /// <summary>
    /// Controller responsible for managing routeinfo-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class RouteInfoController : ControllerBase
    {
        ...
        public RouteInfoController(PatientManagementSystemContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new routeinfo to the database</summary>
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.RouteInfo.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<RouteInfo>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.RouteInfo.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.RouteInfo.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] RouteInfo updatedEntity)
        {
            ...
            this._context.RouteInfo.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<RouteInfo> updatedEntity)
        {
            ...
            var existingEntity = this._context.RouteInfo.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.RouteInfo.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
