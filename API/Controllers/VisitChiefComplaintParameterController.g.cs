...
namespace PatientManagementSystem.Controllers
{
    ...
    public class VisitChiefComplaintParameterController : ControllerBase
    {
        ...
        public VisitChiefComplaintParameterController(PatientManagementSystemContext context)
        {
            _context = context;
        }
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var result = FilterService<VisitChiefComplaintParameter>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<VisitChiefComplaintParameter> updatedEntity)
        {
            ...
            var existingEntity = this._context.VisitChiefComplaintParameter.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.VisitChiefComplaintParameter.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
